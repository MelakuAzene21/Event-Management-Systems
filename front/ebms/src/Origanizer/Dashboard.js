...
const OrganizerDashboard = () => {
    ...
    const [isMenuOpen, setIsMenuOpen] = useState(false); // Profile menu toggle
    const dispatch = useDispatch();
    const user = useSelector((state) => state.auth.user);
    const navigate = useNavigate();
    const notifications = useSelector((state) => state.notifications.notifications);
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case "events":
                return <MyEventsPage />;
            case "tickets":
                return <OriganizerTicketPage />;
            case "booking":
                return <BookingDetail />;
            case "reports":
                return <Reports events={myEvents} />;
            case "notifications":
                return <NotificationsPage />;
            case "reviews":
                return <OrganizerEventReviews />;
            case "vendors":
                return <GetAllVendor />;
            case "profile":
                return <UserProfile />;
            ...
            default:
                return <div className="p-6 text-gray-700">Select a tab from the sidebar.</div>;
        }
    };
    ...
    const tabs = [
        ...
        {
            tab: "vendors",
            icon: "M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v allowableHeight-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z",
            color: "#f97316", // Orange
            ariaLabel: "Manage vendors",
        },
    ];
    ...
    return (
        <div className="min-h-screen flex flex-col bg-gray-50">
            ...
            <header className="fixed top-0 left-0 z-50 w-full bg-white shadow-sm flex justify-between items-center px-4 sm:px-6 py-4">
                <div className="flex items-center gap-3">
                    <button
                        className="md:hidden text-gray-600 hover:text-indigo-900"
                    ...
                    <span className="text-xl font-bold text-indigo-900">EventHub</span>
                </div>
                {user && (
            ...
            {/* Sidebar */}
            <div
                className={`fixed top-16 left-0 h-[calc(100vh-4rem)] bg-gray-50 border-r border-gray-200 shadow-sm overflow-y-auto transition-transform duration-300 ease-in-out z-40
          ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 md:w-64 w-64`}
                ...
                <ul className="space-y-1 px-4 py-6">
                    {tabs.map(({ tab, icon, color, ariaLabel }) => (
                        ...
                        >
                            ...
                            <span className="text-sm font-medium">{tab.charAt(0).toUpperCase() + tab.slice(1)}</span>
                        </li>
                    ))}
                </ul>
            </div>
            ...
            {/* Main Content */}
            <div
                className={`flex-1 h-[calc(100vh-4rem)] overflow-y-auto bg-gray-50 pt-16 transition-all duration-300 ${isSidebarOpen ? "ml-0 md:ml-64" : "ml-0 md:ml-64"
                    }`}
            >
                <div className="p-4 sm:p-6 max-w-7xl mx-auto">
                    <Title title="Dashboard" />
